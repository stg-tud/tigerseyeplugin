	...
	printDeclaredMethods(getDsls().get(combo.getText()));
		...
		if (isCreateConstructors()) {
		    ...
		    logger.warn("Type <" + getGroovySuperClass()
				...
				codeGenerator.addCode(
						...
						CodeGeneration.getMethodComment(project, getTypeName(),
								getPackageText(), getTypeName(), new String[0],
								"V"))
						.addCode("public " + getTypeName() + "(){").addCode(
								IndentationDirection.INDENT_RIGHT,
								CodeGeneration
										.getMethodBodyContent(project,
												getTypeName(), getTypeName(),
												false, "")).addCode(
								IndentationDirection.INDENT_LEFT, "}");
			}
		...
		if (isCreateMain()) {
			codeGenerator.addLineBreak().addCode(
					IndentationDirection.INDENT_RIGHT,
					CodeGeneration.getMethodComment(project, getTypeName(),
							getPackageText(), "main", new String[] { "args" },
							"V")).addCode("public static void main(def args){")
					.addCode(
							IndentationDirection.INDENT_RIGHT,
							CodeGeneration.getMethodBodyContent(project,
									getTypeName(), "main", false, "")).addCode(
							IndentationDirection.INDENT_LEFT, "}");
		} else {
		...
		sourceCode = sourceCode.replaceAll("##imports##", importManager
				.toString());
		sourceCode = sourceCode.replaceAll("##generics##", importManager
				.getGenericTypesAsString());
	...
	Field[] declaredFields = new KeyWordExtractor()
		...
		if (declaredFields != null) {
			for (Field publicDeclaredField : declaredFields) {
				...
				printedLine = publicDeclaredField.getType()
						.getSimpleName()
						+ " " + printedLine;
