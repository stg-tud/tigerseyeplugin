...
/**
 * View for a structured element keyword in the PopartLanguageWizard
 * 
 * @author David Marx
 * @author Thorsten Peter
 */
public class StructuredElementKeywordView extends Composite implements
		DisposeListener {
	...
	public StructuredElementKeywordView(Composite parent, int style,
		...
		radios[0] = new Button(rightComposite, SWT.RADIO);
		radios[0].setSelection(keyword.getParameterStyle()==PopartStructuredElementKeyword.NAMED);
		radios[0].setText("named parameters");
		radios[0].setBounds(10, 5, 75, 30);
		radios[0].addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				Button button = (Button) event.widget;
				if (button.getSelection()) {
					getKeyword().setParameterStyle(
							PopartStructuredElementKeyword.NAMED);					
				}
			}
		});

		radios[1] = new Button(rightComposite, SWT.RADIO);
		radios[1].setText("explitic parameters");
		radios[1].setSelection(keyword.getParameterStyle()==PopartStructuredElementKeyword.EXPLICIT);
		...
		radios[1].addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if (button.getSelection()) {
					getKeyword().setParameterStyle(
							PopartStructuredElementKeyword.EXPLICIT);					
				}
			}
		});
		
		
		createViewer(rightComposite);
		gridData = new GridData(145, 200);
		viewer.getTable().setLayoutData(gridData);

		Composite buttons = new Composite(rightComposite, SWT.NONE);
		...
		remove.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				IStructuredSelection selection = (IStructuredSelection) getParameterViewer()
						.getSelection();
				for (Iterator<PopartParameter> iterator = selection
	...
	private void createColumns(final Composite parent) {
		...
		for (int i = 0; i < titles.length; i++) {
			...
			viewerColumn.setEditingSupport(new ParameterEditingSupport(viewer,
					i));
		}
	...
	protected PopartStructuredElementKeyword getKeyword() {
		return keyword;
	}
